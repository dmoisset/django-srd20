...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for f in orm.Feat.objects.all():
            f.multiple = f.multiple_copy == 'Yes'
            f.stack = f.stack_copy =='Yes'
            f.save()

    def backwards(self, orm):
        for f in orm.Feat.objects.all():
            f.multiple_copy = ('No', 'Yes')[f.multiple] # This needs to run in python 2.5, so no if/else expressions
            f.stack_copy = ('No', 'Yes')[f.stack]
            f.save()
